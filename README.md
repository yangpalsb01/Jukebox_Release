...
> ⚠️ **주의:** 업로드할 때 파일 목록에 `.env` 라는 파일이 보이면 반드시 제외하세요.  
> 이 zip에는 `.env` 파일이 없으니 정상적으로 압축을 풀었다면 신경 쓰지 않아도 됩니다.
...
2. **Create a deployment** → **M0 (무료)** 선택 후 클러스터를 생성하세요.
3. 생성 중에 **Username**과 **Password**를 설정하라고 나옵니다. 기억해두세요.
4. 클러스터 생성이 완료되면 **Connect** 버튼 클릭 → **Drivers** 선택
5. 아래와 같이 생긴 연결 문자열을 복사하세요:
   ```
   mongodb+srv://<username>:<password>@cluster0.xxxxx.mongodb.net/?retryWrites=true&w=majority
   ```
6. 복사한 문자열에서 `<username>`과 `<password>` 부분을 3번에서 설정한 값으로 교체하세요.
7. 왼쪽 메뉴 **Network Access** → **Add IP Address** → **Allow Access from Anywhere** 선택 후 저장하세요.
...
6. **Create Web Service** 클릭하면 배포가 시작됩니다.
7. 배포 완료 후 `https://your-app.onrender.com` 주소로 접속하세요.
...
- Render.com 무료 플랜은 15분간 접속이 없으면 서버가 잠시 잠듭니다.  
  다음 접속 시 30초~1분 정도 느릴 수 있습니다. (셀프 핑 기능으로 완화됩니다)
